-- Corrective migration: fix edit_venue array append operator
-- Root cause: v_mef := v_mef || 'fieldname' in the original migration causes PostgreSQL
--   to resolve || as text[] || text[] and attempt to parse 'fieldname' as an array literal,
--   producing 22P02 "malformed array literal" on any real-change PATCH call.
-- Fix: replace all 6 occurrences with array_append(v_mef, 'fieldname'),
--   which is unambiguous and never triggers overload resolution.
-- Identical to 20260627_edit_venue_rpc.sql except for those 6 lines.
