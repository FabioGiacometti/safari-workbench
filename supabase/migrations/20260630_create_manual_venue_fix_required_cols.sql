-- Migration: 20260630_create_manual_venue_fix_required_cols.sql
-- Fixes create_manual_venue to supply the two NOT NULL columns that have no DB default:
--   • fingerprint  = slugify(canonical_name) || '::' || slugify(city)
--                    (falls back to slugify(canonical_name) || '::manual' when city is NULL)
--   • display_name = canonical_name  (pipeline always mirrors canonical_name here)
--
-- Slug format matches existing venues: lowercase, accents stripped (unaccent), spaces/special
-- chars replaced with hyphens, consecutive hyphens collapsed, leading/trailing hyphens removed.
--
-- Fingerprint uniqueness: if the generated fingerprint already exists a counter suffix is
-- appended ( ::2, ::3, …) to prevent constraint violations on repeated manual creations
-- of venues with the same name+city. This mirrors what the pipeline would do.
--
-- ROLLBACK:
--   Re-apply 20260630_create_manual_venue.sql
--   (CREATE OR REPLACE replaces the function in place)
...
BEGIN
  ...
  -- Format: slugify(canonical_name)::slugify(city) — mirrors pipeline convention.
  -- slugify: unaccent, lowercase, replace non-alphanumeric with hyphen, collapse hyphens.
  -- When city is NULL, uses 'manual' as city segment to still produce a unique-enough base.
  v_fp_base := regexp_replace(
                 regexp_replace(
                   lower(unaccent(v_canonical_name)),
                   '[^a-z0-9]+', '-', 'g'
                 ),
                 '^-+|-+$', '', 'g'
               )
  ...
  -- Ensure fingerprint uniqueness — append counter suffix if needed.
  ...
  VALUES (
    v_canonical_name,
    v_canonical_name,   -- display_name mirrors canonical_name at creation
    v_fingerprint,
    COALESCE(v_aliases, '{}'),
    v_city,
    v_region,
...
-- ─── Permissions (re-apply — CREATE OR REPLACE preserves grants but be explicit) ──
