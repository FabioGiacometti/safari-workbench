-- Migration: 20260628_edit_venue_geo_entity.sql
-- Extends edit_venue to accept geo_entity_id as an explicit operator field.
--
-- Changes from 20260627_fix_edit_venue_array_append.sql:
--   • geo_entity_id added to v_allowed_keys
--   • geo_entity_id extracted, validated (entity must exist + be active), diff'd, and updated
--   • geo_entity_id tracked in manually_edited_fields
--   • geo_entity_id change written to venue_edit_log (same as other fields)
--   • geo_entity_id change additionally written to editorial_actions for auditability
--
-- Invariants preserved:
--   • geo_entity_id is NEVER derived automatically from lat/lng or city text
--   • Only an explicit operator-supplied value (from a selector) is accepted
--   • Unknown or inactive entity IDs are rejected before any DB write
--   • All existing field protections are unchanged
--
-- ROLLBACK:
--   Re-apply 20260627_fix_edit_venue_array_append.sql
--   (DROP FUNCTION IF EXISTS public.edit_venue(uuid, jsonb, text) is also safe)
...
DECLARE
  ...
  v_has_geo_entity_id  boolean := FALSE;            -- NEW
  ...
  -- geo_entity_id added to allowed keys
...
BEGIN
  ...
  -- geo_entity_id: operator-explicit only. Never derived from lat/lng or city.
  -- Null is accepted (clears the link). Non-null must reference an active entity.
  IF p_fields ? 'geo_entity_id' THEN
    ...
    ELSE
      ...
      END IF;
      -- Validate existence and active status before any write
      IF NOT EXISTS (
  ...
  IF v_has_canonical_name AND (v_canonical_name IS DISTINCT FROM v.canonical_name) THEN
    ...
    IF NOT ('canonical_name' = ANY(v_mef)) THEN v_mef := array_append(v_mef, 'canonical_name'); END IF;
  END IF;
  IF v_has_aliases AND (v_aliases IS DISTINCT FROM v.aliases) THEN
    ...
    v_changes := v_changes + 1;
    IF NOT ('aliases' = ANY(v_mef)) THEN v_mef := array_append(v_mef, 'aliases'); END IF;
  END IF;
  IF v_has_city AND (v_city IS DISTINCT FROM v.city) THEN
    ...
    v_changes := v_changes + 1;
    IF NOT ('city' = ANY(v_mef)) THEN v_mef := array_append(v_mef, 'city'); END IF;
  END IF;
  IF v_has_region AND (v_region IS DISTINCT FROM v.region) THEN
    ...
    v_changes := v_changes + 1;
    IF NOT ('region' = ANY(v_mef)) THEN v_mef := array_append(v_mef, 'region'); END IF;
  END IF;
  IF v_has_lat AND (v_lat IS DISTINCT FROM v.lat) THEN
    ...
    v_changes := v_changes + 1;
    IF NOT ('lat' = ANY(v_mef)) THEN v_mef := array_append(v_mef, 'lat'); END IF;
  END IF;
  IF v_has_lng AND (v_lng IS DISTINCT FROM v.lng) THEN
    ...
    v_changes := v_changes + 1;
    IF NOT ('lng' = ANY(v_mef)) THEN v_mef := array_append(v_mef, 'lng'); END IF;
  END IF;
  ...
  IF v_has_accessibility AND (v_accessibility IS DISTINCT FROM v.accessibility) THEN
    ...
    v_changes := v_changes + 1;
  END IF;
  IF v_has_geo_entity_id AND (v_geo_entity_id IS DISTINCT FROM v.geo_entity_id) THEN
    v_diff    := v_diff || jsonb_build_object('field','geo_entity_id','old',to_jsonb(v.geo_entity_id),'new',to_jsonb(v_geo_entity_id));
    v_changes := v_changes + 1;
    IF NOT ('geo_entity_id' = ANY(v_mef)) THEN v_mef := array_append(v_mef, 'geo_entity_id'); END IF;
  END IF;
  ...
  -- 8. City mismatch warning (fires only when canonical_city_id is populated)
  ...
  UPDATE venues SET
    ...
    geo_entity_id          = CASE WHEN v_has_geo_entity_id    THEN v_geo_entity_id     ELSE geo_entity_id          END,
    manually_edited_fields = v_mef,
    updated_at             = now()
  WHERE id = p_venue_id;

  -- 10. INSERT audit rows into venue_edit_log — one per changed field
  ...
  -- 11. Additional editorial_actions row when geo_entity_id changes
  --     (provides an explicit audit trail separate from venue_edit_log)
  IF v_has_geo_entity_id AND (v_geo_entity_id IS DISTINCT FROM v.geo_entity_id) THEN
    INSERT INTO editorial_actions(actor, action_type, entity_type, entity_id, after_state)
    VALUES (
      p_actor,
      'venue_geo_entity_set',
      'venue',
      p_venue_id::text,
      jsonb_build_object(
        'old_geo_entity_id', v.geo_entity_id,
        'new_geo_entity_id', v_geo_entity_id,
        'venue_id',          p_venue_id
      )
    );
  END IF;
...
-- SELECT id, display_name, status FROM geo_entities LIMIT 1;  -- confirm geo_entities.status column exists
-- SELECT edit_venue('<any-venue-uuid>', '{"geo_entity_id": "geo::city::cordoba-ar"}', 'test@example.com');
-- ROLLBACK;  -- if just testing
